...
namespace Webkul\Support\Navigation;
...
class ModuleAwareNavigationManager extends NavigationManager
{
    ...
    public function get(): array
    {
        ...
        return collect($this->getNavigationItems())
            ...
            ->filter(function (NavigationGroup $group, string $groupIndex): bool {
                if (blank($group->getItems())) {
                    return false;
                }

                $groupName = unserialize($groupIndex);

                if ($groupName instanceof UnitEnum && method_exists($groupName, 'isActive')) {
                    return $groupName->isActive();
                }
